...
const CONTACT_URL = `../app/assets/datas/contacts.json`;
...
export class ContactService {
    ...
    request(url: string, opts: any) {
        let arr: any = this.http.request(url, new RequestOptions(opts));
        return arr.map((res: any) => {
            let _res = res.json();
            if (opts.id) {
    ...
    addContact(obj: Object = {}) {
        ...
        return this.http.post(CONTACT_URL, body, options).subscribe((res: any) => res.json());
    }
}
